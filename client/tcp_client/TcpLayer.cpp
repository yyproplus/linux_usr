...
{
    int ret=client_.Init(socket_type,port);
    if(ret<0){
...
void TcpLayer::TcPLayerSendThread(){
    ...
    uint8_t data=0x01;
    int ret=0;
    select_status_=client_.SelectNoblockMode();
    sleep(2);
...
{
    SelectModeReturnCode select_status_=client_.SelectNoblockMode();
    if(select_status_==SELECT_READ_ONLY||select_status_==SELECT_RW){
        memset(pdata,0,sizeof(pdata));
        len_=client_.Read(pdata,sizeof(pdata));
        if (len_ < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                // 套接字暂时没有数据可读，稍后重试
